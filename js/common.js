...
var jsApi = [],jsApiList = [
	...
	'http://utao520.22web.org/m3u8.php?url=',
...
$(function(){
	...
	$('#searchDo').tap(function(){
		...
		if(location.href.indexOf('http://402i6913d8.wicp.vip/list/') < 0){	// 判断当前是否在list目录
...
function parseTemplate(template,data){
	...
	if(data.id){
		if(location.href.indexOf('http://402i6913d8.wicp.vip/list/') < 0){	// 判断当前是否在list目录
			...
			data.href = 'http://402i6913d8.wicp.vip/item.html?vid=' + encodeURIComponent(data.id);
		}
	}
	if(data.from){
