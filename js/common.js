...
$(function(){
	...
	$('#searchDo').tap(function(){
		...
		if(location.href.indexOf('http://utao520.22web.org/list/') < 0){	// 判断当前是否在list目录
...
function parseTemplate(template,data){
	...
	if(data.id){
		if(location.href.indexOf('http://utao520.22web.org/list/') < 0){	// 判断当前是否在list目录
			...
			data.href = 'http://utao520.22web.org/item.html?vid=' + encodeURIComponent(data.id);
		}
	}
	if(data.from){
