...
function searchBox() {
    ...
    '       <label><input type="radio" name="searchDo" value="ITV"> ITV</label>' + 
...
function pageLoad(){
	...
	$(function(){
		if($.URI.keyword){
			...
			$('#keywordItem').html($('#keywordItem').html().replace('{{keyword}}',$.URI.keyword));
...
if(typeof(pageLoaded) == 'undefined' && typeof(jsApi) != 'undefined'){ pageLoad(); }
	$('button').click(function(){
		var data = $('[name="wd"]').val();
		if(data!=''){
			window.location.href = '../list/search.html?keyword=' + data + '';
		}
	});
